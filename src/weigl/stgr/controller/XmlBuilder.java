...
public final class XmlBuilder extends BuilderImpl {
  ...
  public StgrModel parse(final String content) throws BuildException {
    try {
      ...
      String text = "Der Text ist kein gültiges XML. Der Parser meldete folgendes:\n";
      text += e.getMessage();
      final BuildException ex = new BuildException(text, e);
      throw ex;
  ...
  private void parseCmd(final StgrCommandBlock parent, final Element e) {
    ...
    while (iter.hasNext()) {
      ...
      if (current.getName().equalsIgnoreCase("do")) {
        parent.append(new StgrCommand(current.getAttributeValue("label")));
        ...
        final StgrRepeat r = new StgrRepeat(current.getAttributeValue("label"));
        parseCmd(r.getCommands(), current);
        parent.append(r);
      } else if (current.getName().equalsIgnoreCase("while")) {
        final StgrWhile w = new StgrWhile(current.getAttributeValue("label"));
        parseCmd(w.getCommands(), current);
        parent.append(w);
      } else if (current.getName().equalsIgnoreCase("switch")) {
        final StgrSwitch w = new StgrSwitch(current.getAttributeValue("label"));
        parseCmd(w.getCommands(), current);
        parent.append(w);
      } else if (current.getName().equalsIgnoreCase("while")) {
        final StgrCase w = new StgrCase(current.getAttributeValue("label"));
        parseCmd(w.getCommands(), current);
        parent.append(w);
      } else if (current.getName().equalsIgnoreCase("call")) {
        final StgrCall call = new StgrCall(current.getAttributeValue("label"));
        parent.append(call);
      }
    }
  }
