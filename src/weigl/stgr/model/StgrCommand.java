...
public class StgrCommand implements ICommand {
	...
	public void paintOn(Graphics2D g, int x, int y, int w, int h) {
		...
		g.drawString(this.m_label, x + 5, y + getSize().height - 5);
	}

	public Dimension getSize(FontMetrics f) {
		System.out.println("Command.getDimension()");
		m_size.width = 10 + getLabelWidth(f);
		m_size.height = 10 + getLabelHeight(f);
		return m_size;
	}
